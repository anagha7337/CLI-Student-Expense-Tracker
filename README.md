...
1. Save the program as expense_tracker.py.
2. Open a terminal in the file's directory.
3. Run the command:
```bash
  python expense_tracker.py
```
4. Use the menu options displayed on the screen to manage expenses.
...
If I had more time, I would add file-based storage (using JSON or CSV) so that expenses are automatically saved and loaded between program runs instead of being reset each time the program starts.
